...
class Config:
    def __init__(self) -> None:
        ...
        # Создаём директорию для settings.json (монтируется в Docker)
        ...
        self.SNAPSHOT_IMPORT_TASKS_ENDPOINT: str = ""
        ...
        # Временные директории в /tmp (очищаются при рестарте)
        import tempfile
        self.TMP_DIR: Path = Path(tempfile.gettempdir()) / "ptaf-tools"
        self.SNAPSHOTS_DIR: Path = self.TMP_DIR / "snapshots"
        self.RULES_DIR: Path = self.TMP_DIR / "rules"
        self.ACTIONS_DIR: Path = self.TMP_DIR / "actions"
        self.GLOBAL_LISTS_DIR: Path = self.TMP_DIR / "global_lists"
    ...
    def reload_from_sources(self) -> None:
        ...
        self.SNAPSHOT_IMPORT_TASKS_ENDPOINT = self._settings_or_env(
            "SNAPSHOT_IMPORT_TASKS_ENDPOINT",
            "SNAPSHOT_IMPORT_TASKS_ENDPOINT",
            f"{self.API_PATH}/config/snapshot_import_tasks"
        )
